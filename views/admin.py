...
def render_admin_tools(username: str) -> None:
    st.caption("인원 명부·본사 요청 배포·취합은 여기에서 합니다. 특근 입력은 달력에서 언제든지 가능합니다.")
    roster_tab, hq_tab, survey_tab, review_tab, account_tab = st.tabs(
        ["인원 명부", "본사 요청 양식", "조사 관리", "제출 현황 · 취합", "계정"]
    )
    with roster_tab:
