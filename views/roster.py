...
def _render_admin_roster(username: str) -> None:
    st.subheader("인원 명부")
    st.caption(
        "최고 관리자는 전체 인원을 보고, 엑셀 양식에 기입한 뒤 업로드해 명부를 바꿀 수 있습니다. "
        "팀은 에이텍모빌리티·에이텍컴퓨터 인원을 올립니다."
    )
    st.write(
    ...
    with down_col:
        st.download_button(
            ...
            file_name="인원명부_양식.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="dl_roster_template",
        )
        st.caption("빈 양식입니다. 성명·회사·팀·고용형태를 채운 뒤 업로드하세요.")
    with current_col:
        st.download_button(
            "현재 전체 명부 엑셀 받기",
            data=build_employee_template(current),
            file_name="인원명부_전체.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="dl_roster_current",
            disabled=not current,
        )
        if current:
            st.caption("지금 저장된 전체 인원을 받아 수정한 뒤 다시 올릴 수 있습니다.")
        else:
            st.caption("아직 저장된 명부가 없습니다.")

    uploaded = st.file_uploader("작성한 명부 엑셀 업로드 (전체 교체)", type=["xlsx"], key="up_roster")
    ...
    if not current:
        st.info("아직 명부가 없습니다. 인원 양식을 받아 성명·회사·팀·고용형태를 채운 뒤 업로드하세요.")
        st.caption(
